...
for WEACAST_MODEL in $WEACAST_MODELS; do
  ...
  docker build --pull --build-arg TAG=$WEACAST_LOADER_TAG --build-arg SYNC_DB_URL=$WEACAST_DB_URL -f dockerfile.$WEACAST_MODEL -t $WEACAST_MODEL-loader:$WEACAST_LOADER_TAG $GIT_URL
